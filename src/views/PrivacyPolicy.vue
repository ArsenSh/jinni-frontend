  ...
  <div class="legal-page" :class="currentTheme">
    ...
    <header class="legal-header">
      ...
      <div class="header-content">
        ...
        <p class="page-subtitle">{{ $t('privacy.last_updated') }}</p>
        ...
        <div class="legal-content">
          ...
          <section class="legal-section">
            ...
            <div class="subsection">
              <h3>{{ $t('privacy.sections.collection.subsections.account.title') }}</h3>
              <p>{{ $t('privacy.sections.collection.subsections.account.content') }}</p>
            </div>
            <div class="subsection">
              <h3>{{ $t('privacy.sections.collection.subsections.preferences.title') }}</h3>
              <p>{{ $t('privacy.sections.collection.subsections.preferences.content') }}</p>
            </div>
            <div class="subsection">
              <h3>{{ $t('privacy.sections.collection.subsections.location.title') }}</h3>
              <p>{{ $t('privacy.sections.collection.subsections.location.content') }}</p>
            </div>
            <div class="subsection">
              <h3>{{ $t('privacy.sections.collection.subsections.usage.title') }}</h3>
              <p>{{ $t('privacy.sections.collection.subsections.usage.content') }}</p>
            </div>
            <div class="subsection">
              <h3>{{ $t('privacy.sections.collection.subsections.device.title') }}</h3>
              <p>{{ $t('privacy.sections.collection.subsections.device.content') }}</p>
            </div>
          </section>
          ...
          <section class="legal-section">
            ...
            <ul class="legal-list">
              <li v-for="(point, index) in $tm('privacy.sections.use.points')" :key="index">{{ point }}</li>
            </ul>
          </section>
          <!-- Information Sharing -->
          <section class="legal-section">
            ...
            <div class="subsection">
              <h3>{{ $t('privacy.sections.sharing.subsections.service.title') }}</h3>
              <p>{{ $t('privacy.sections.sharing.subsections.service.content') }}</p>
            </div>
            <div class="subsection">
              <h3>{{ $t('privacy.sections.sharing.subsections.legal.title') }}</h3>
              <p>{{ $t('privacy.sections.sharing.subsections.legal.content') }}</p>
            </div>
            <div class="subsection">
              <h3>{{ $t('privacy.sections.sharing.subsections.business.title') }}</h3>
              <p>{{ $t('privacy.sections.sharing.subsections.business.content') }}</p>
            </div>
          </section>
          <!-- AI and Data Processing -->
          <section class="legal-section">
            <h2>{{ $t('privacy.sections.ai.title') }}</h2>
            <p>{{ $t('privacy.sections.ai.content') }}</p>
          </section>
          ...
          <section class="legal-section">
            ...
            <ul class="legal-list">
              <li v-for="(point, index) in $tm('privacy.sections.rights.points')" :key="index">{{ point }}</li>
            </ul>
          </section>
    ...
    <footer class="legal-footer">
      <div class="footer-content">
        ...
        <div class="footer-links">
          <a href="/terms" class="footer-link">{{ $t('landing.footer.terms') }}</a>
        </div>
...
export default {
  ...
  methods: { 
    ...
    closeLangDropdown() { this.langOpen = false },
    selectLang(code) {
      this.$i18n.locale = code
      this.langOpen = false
      // Persist the language choice so loadLanguage() doesn't overwrite it
      try {
        const savedSettings = localStorage.getItem('jinni_settings')
    ...
    loadLanguage() {
      ...
      if (authToken) {
        // Authenticated user - use their saved language preference
        const savedSettings = localStorage.getItem('jinni_settings');
        if (savedSettings) {
          try {
            const settings = JSON.parse(savedSettings);
        ...
        // Unauthenticated user - use browser language or keep current
...
<style scoped>
.legal-page{min-height:100vh;transition:background 0.5s ease,color 0.3s ease;font-family:'Segoe UI',system-ui,-apple-system,sans-serif;display:flex;flex-direction:column;position:relative;z-index:1}
...
.subsection{margin-top:1.5rem;padding-left:1rem;border-left:3px solid rgba(139,92,246,0.3)}
.subsection h3{font-size:1.15rem;font-weight:600;margin-bottom:0.75rem}
.subsection p{font-size:0.98rem;line-height:1.7}
...
.night-mode .subsection{border-left-color:rgba(139,92,246,0.4)}
.night-mode .subsection h3{color:#c084fc}
.night-mode .subsection p{color:#e2e8f0}
...
.day-mode .subsection{border-left-color:rgba(212,175,55,0.5)}
.day-mode .subsection h3{color:#A0522D}
.day-mode .subsection p{color:#3c2a1e}
...
@media (max-width:768px){
  ...
  .subsection{padding-left:0.75rem}
