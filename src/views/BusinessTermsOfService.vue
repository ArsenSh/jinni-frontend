  ...
  <div class="legal-page" :class="currentTheme">
    ...
    <header class="legal-header">
      ...
      <div class="header-content">
        <h1 class="page-title">{{ $t('businessTerms.title') }}</h1>
        <p class="page-subtitle">{{ $t('businessTerms.last_updated') }}</p>
      ...
      <div class="legal-container">
        <div class="legal-content">
          ...
          <section class="legal-section">
            <h2>{{ $t('businessTerms.sections.intro.title') }}</h2>
            <p>{{ $t('businessTerms.sections.intro.content') }}</p>
          </section>

          <!-- ── Tier selector ─────────────────────────────────────── -->
          <section class="legal-section tier-selector-section">
            <h2>{{ $t('businessTerms.tier_selector.title') }}</h2>
            <p>{{ $t('businessTerms.tier_selector.subtitle') }}</p>
            <div class="tier-buttons">
              ...
              >
                ...
                {{ $t('businessTerms.tiers.' + tier + '.label') }}
              </button>
            </div>
          </section>
          ...
          <section class="legal-section tier-panel" :class="activeTier" ref="tierPanel">
            <h2>{{ $t('businessTerms.tiers.' + activeTier + '.title') }}</h2>
            <p>{{ $t('businessTerms.tiers.' + activeTier + '.intro') }}</p>
            <ul class="legal-list">
              <li v-for="(point, index) in $tm('businessTerms.tiers.' + activeTier + '.points')" :key="index">
                {{ tt(point) }}
              </li>
            </ul>
          </section>

          <!-- ── Terms that apply to every business listing ────────── -->
          <section
            v-for="(section, sIndex) in $tm('businessTerms.common')"
            :key="sIndex"
            class="legal-section"
          >
    ...
    <footer class="legal-footer">
      <div class="footer-content">
        ...
        <div class="footer-links">
          <a href="/business/privacy" class="footer-link">{{ $t('landing.footer.privacy') }}</a>
        </div>
...
export default {
  ...
  mounted() {
    this.loadLanguage()
    // Allow deep-linking straight to a tier, e.g. /business/terms?tier=signature
