...
export default {
  ...
  setup() {
    ...
    const PRIMARY_CATS = ['restaurants','hotels','events','historical','hidden_gems','activities','souvenirs','clothing','jewelry','food']
    ...
    const zoneRadiusLabel = computed(() => {
      const cat = business.value?.type?.find(t => PRIMARY_CATS.includes(t))
      const map = { restaurants: '300m', hotels: '900m', events: '300m', historical: '500m', hidden_gems: '900m', activities: '900m', souvenirs: '300m', clothing: '300m', jewelry: '300m', food: '300m' }
      return cat ? (map[cat] || '—') : '—'
    })
    ...
    async function saveEdits() {
      ...
      try {
        const user = getStoredUser()
        const b = business.value
        // SUPERSET — this decides which primaries survive the owner's save.
        // A key missing here is silently dropped from Business.type[].
        const PRIMARY_CATS_SET = new Set(['restaurants','hotels','events','historical','hidden_gems','activities','souvenirs','clothing','jewelry','food'])
